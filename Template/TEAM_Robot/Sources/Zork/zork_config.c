...
#if PL_CONFIG_HAS_RTOS
	TaskHandle_t ZorkPlayTaskHandle;
#endif
...
TaskHandle_t Get_ZorkTaskHandle(void) {
	return ZorkPlayTaskHandle;
}

/* Task for play the Game */
static void ZorkPlayTask(void){
	vTaskSuspend(Get_ZorkTaskHandle());
	if (run_zork_game() != 0) {
		vTaskResume(Get_ShellTaskHandle());
	}
	vTaskDelete(NULL);
}
...
void zork_init(void){
	/* Task for the game ZORK */
	if (xTaskCreate(ZorkPlayTask,
					"Zork",
					800/sizeof(StackType_t),
					(void*) NULL,
					tskIDLE_PRIORITY,
					&ZorkPlayTaskHandle) != pdPASS) {
		for (;;) {/*Test*/
		}
	} // end task: Zork play Task
}
